...
```
$ docker-compose exec iris iris session iris -U IRISAPP
IRISAPP>write ##class(adyubaylo.sample.ObjectScript).Test()
It works!
42
```
...
2. Class `adyubaylo.sample.PersistentClass` contains a method `CreateRecord` that creates an object with one property, `Test`, and returns its id.

Open IRIS terminal and run:

```
IRISAPP>write ##class(adyubaylo.sample.PersistentClass).CreateRecord(.id)
1
IRISAPP>write id
1
```
...
```
IRISAPP>write ##class(adyubaylo.sample.PersistentClass).ReadProperty(id)
Test string
```
